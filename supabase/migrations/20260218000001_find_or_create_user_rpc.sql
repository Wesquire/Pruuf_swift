-- Migration: Create find_or_create_user RPC function
-- This function bypasses RLS to handle user lookup and creation/reassociation
-- when a user re-authenticates with a new anonymous session (new auth UUID)
...
BEGIN
    ...
    IF p_phone IS NOT NULL AND p_phone != '' THEN
        ...
        IF FOUND THEN
            ...
            UPDATE users
            SET id = p_auth_id, updated_at = v_now
            WHERE id = v_user.id
